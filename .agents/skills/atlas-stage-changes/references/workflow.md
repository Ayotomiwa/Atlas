...
Delegate read-only interpretation to `atlas-change-analyst`; the skill owns decisions, interaction, staging and checkpoint advancement. Preview a matrix of logical change, commit/MR, affected candidates, finding, outcome, staging boundary and gap before any write.
