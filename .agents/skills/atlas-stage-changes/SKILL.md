...
5. Show one persistence preview covering exact staging files and checkpoint compare-and-swap. After approval, complete the shared taxonomy/contract and destination-contract reads, write coherent `staging.change` records, lint, update the eligible cursor through the helper and lint again. Deferred items remain unresolved and unassessed/concurrent/ambiguous/rewritten ranges block advancement.
