...
5. For durable evidence, show the proposed bucket/title, qualified question IDs, bounded claims, provenance, remaining uncertainty, duplicate/pending routes and exclusions. Build the ephemeral duplicate snapshot defined by the shared handoff contract. Write nothing until explicit approval, then pass the approved preview, evidence unit and snapshot to `atlas-stage`; it must not ask again. Do not remove the curated question directly.
