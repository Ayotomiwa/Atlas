...
5. Allocate an ID using `STG-YYYYMMDD-<slug>`; scan existing staging IDs and add `-2`, `-3`, and so on deterministically for same-day collisions. Put component/flow evidence under a supported registered candidate domain or `unassigned` when the domain is uncertain.
...
7. Preserve uncertainty; never create reviewed or authoritative typed connections/links here.
...
11. Run `python scripts/atlas_lint.py .` after writing unless validation is explicitly deferred for the iteration.
