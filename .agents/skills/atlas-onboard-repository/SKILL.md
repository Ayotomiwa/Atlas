...
1. Establish one logical boundary, physical Git root/remote, candidate `repository_root`, enclosing boundary, scope and exclusions. Check existing coverage with `context`.
...
5. Stage one repository/component discovery record. Add other buckets only for independently evidenced boundaries; never placeholders or curated facts.
6. Report records, evidence matrix, full scan manifest, exclusions/inaccessible paths/stopping reasons, possible findings, questions and validation with references.
