...
Read shared human-intent/persistence/runtime/provenance/handoff references, `references/full-baseline.md`, the clarification checklist, staging root policy and every potentially relevant bucket contract. This is full-baseline **Sync Atlas**.
...
8. Show one batch preview including exclusions, unresolved lenses, source snapshot and future intake anchor. After one approval, write the unchanged batch without repeated handoff approval. Do not advance `_intake/`.
