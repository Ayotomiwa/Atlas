...
Read shared persistence/runtime/provenance and `assets/managed-block.md`.
...
4. Complete Git preflight in the product repository. Run `scripts/manage_agent_block.py inspect` and `dry-run` with the Codex asset. Stop on malformed markers; preview branch, target, diff and local commit before applying.
5. Preserve every byte outside markers, omit absolute checkout paths and unrelated instructions, create one exact-path local commit, and show branch/commit plus the repository-relative diff. Require an advisory whenever a `not-verified` seed is used.
