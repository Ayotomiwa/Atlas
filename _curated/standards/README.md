...
The root and category catalogues are generated by `python scripts/rebuild_atlas.py` from the pages themselves, and `python scripts/rebuild_atlas.py --check` reports drift, so do not hand-edit them. Archived standards are excluded automatically and remain in place and in history. Keep the surrounding routing prose and coverage notes current by hand.
