...
Reuse selected IDs, opened pages and source reads supplied in the handoff. Batch independent reads only within an already selected scope, and return appended ordered access events so the parent derives route and fallback wording from actual access.
