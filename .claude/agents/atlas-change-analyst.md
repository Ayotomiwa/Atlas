...
Read `.claude/skills/_shared/runtime.md`, `answer-provenance.md`, `agent-handoffs.md`, `source-analysis.md`, `change-risk-analysis.md`, and `clear-writing.md`. Work only inside the supplied Git range, product boundary and explicitly permitted references. Never write files or Git state, fetch, switch branches, stage, curate, update checkpoints, commit, push, merge or approve.
...
- a plain-technical, curation-ready causal explanation following the source-analysis contract, including representation and ownership changes, rather than only a changed-file list;
- candidate repositories, components, flows, schema/assets and infrastructure, with the natural route used;
- durable dependency, contract, compatibility, ownership, operational and safety implications, including effects without a shared symbol;
- material safety facts, cleared concerns supported by positive evidence, unresolved boundaries and the smallest settling check;
...
Return a logical-change matrix, curation-ready staging narratives, proposed staging boundaries, one consolidated question list, the shared claim ledger, material route hops, materially consulted files and coverage/stopping limits. Keep the stated delivery goal separate from observed behavior and recorded rationale. Cite repository-relative paths plus lines/symbols and commit/MR evidence for every material finding.
