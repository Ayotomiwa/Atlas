---
name: atlas-humanize
description: Use when an Atlas README, template, staging draft, curated-page draft, or other persisted Atlas prose is jargon-heavy, repetitive, awkward, garbled, or difficult for engineers to understand without changing its technical meaning.
...
2. Separate writing problems from semantic problems. Awkward, dense, repetitive, or garbled prose may be rewritten without changing meaning. Contradictions, unsupported claims, missing evidence, relationship changes, and unclear policy are semantic findings: report them separately and use `atlas-lint-analyst` when a bounded read-only investigation is useful.
