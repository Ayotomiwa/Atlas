...
1. Read staging policy, taxonomy/statuses, `references/semantic-preflight.md`, and each source record. Use `python scripts/atlas_query.py staging --format json` for the complete cross-bucket active queue, then process coherent related `status: new` batches; check merged queue status and active work, and mark the selected records `curating` while reconciliation and review are active. Never mutate committed staging beyond top-level `status`.
