...
Read `../_shared/human-intents.md`, `../_shared/runtime.md`, `../_shared/answer-provenance.md`, `../_shared/agent-handoffs.md`, `../_shared/source-analysis.md`, and `../_shared/diagram-writing.md` before substantive work. This is the normal **Ask Atlas** route and is read-only.
...
5. Handle only a small direct lookup as defined by the shared handoff contract locally. Delegate synthesis, unresolved ambiguity, traversal, history, or substantial source fallback to `atlas-discovery-analyst`.
...
Apply the shared provenance rules to uncertainty, executable claims, source revision, source reuse, and coverage. Use the source-analysis contract for every fallback, architecture explanation, or rationale question. Keep current implementation, documented intent, recorded rationale, inference, and recommendation separate.
...
When the request needs a complete flow or readiness result and Atlas coverage is partial or uncertain, apply bounded source fallback under `source-analysis.md` to the missing flow edge rather than presenting the partial route as complete. If the edge cannot be verified, keep it qualified and state the unresolved boundary.
