...
For a write-capable specialist, also pass the exact persistence preview, the user's approval, permitted files/claims/status effects, required contract reads and the work-guard scope. A curator receives no generation, validation, repair, review or lifecycle ownership: it returns materialized paths and a claim ledger for the parent checkpoint and scoped validation. The specialist must not expand scope or ask for approval again; it returns a material change to the parent for a revised preview.
