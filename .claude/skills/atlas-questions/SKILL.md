...
6. When durable evidence exists, read `references/staging-handoff.md`, build its ephemeral duplicate snapshot and show the staging preview. Do not write anything until the user explicitly approves it.
7. After approval, invoke `atlas-stage` with the approved preview, evidence unit and duplicate snapshot. The staging workflow owns persistence and validation and must not ask for the same approval again.
