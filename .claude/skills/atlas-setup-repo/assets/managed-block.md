...
Presence of this managed block binds this repository to Atlas for automatic routing. **Explicit Ask Atlas** consults Atlas even from an unbound repository; an unbound repository does not invoke it automatically. Here, use one known targeted source read for a clearly local question, but route an uncertain, broad, multi-hop, Git-history, or durable-context lookup through Atlas before broad source search. Preserve connection confidence marked possible, unconfirmed, or conflicting: qualify it or verify precisely the uncertain source edge before a definitive, executable or complete claim. Treat external targets and unknown coverage as separate states. Run semantic-risk change readiness regardless of diff size; a genuinely local isolated edit may remain source-only.
...
Within one conversation, keep only ephemeral Atlas session context: Atlas/product roots and current path, selected stable IDs and already-opened pages, inspected source paths, the coverage endpoint, whether the checkout advisory was disclosed, and an ordered record of actual retained/bootstrap/Atlas/source accesses. Reuse retained context while repository, record/evidence and question type are unchanged. Batch independent Atlas reads of already-selected evidence and combine bounded source checks that share one authorised boundary; derive route/fallback descriptions from actual accesses. Re-enter Atlas for a repository/record change, suspected source/checkout change, the end of recorded coverage, or impact, ownership, conflict, standards, recovery or other cross-boundary work; carry still-valid context through internal handoffs. New conversations start cold and persist none of this state.
