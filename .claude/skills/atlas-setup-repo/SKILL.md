...
4. Run `scripts/manage_agent_block.py inspect`, then `dry-run` with this skill's asset. Stop on malformed markers. Apply only after direct invocation or the user's accepted setup offer.
...
6. Show the repository-relative target and exact diff. Explain that a `not-verified` seed remains a routing candidate and requires an advisory in every answer that uses it.
