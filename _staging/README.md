...
Use `_staging/index.md` to route to buckets and `python scripts/atlas_query.py staging` for the single read-only queue view across every bucket. Its component, flow and domain filters replace generated component/flow catalogues. Infrastructure remains a generated root/domain queue because those records are domain-routed; other buckets continue to use Git/search rather than manually maintained catalogues.
